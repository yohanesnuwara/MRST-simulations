###########################################################################
#    Two-phase Incompressible and Immiscible Simulation in 2D Reservoir   #
#        Waterflooding in Dead (non-volatile; non-gas-saturated) Oil      #
#                                                                         #
#                      Buckley-Leverett Simulation                        #
###########################################################################

%% Simulates Buckley-Leverett 2D displacement. Observe the fingering of water
%% at water breakthrough, and observe how the plot of Sw and cum. prod looks like. 
...
%W = addWell(W,G, rock, (G.cells.num/2)+0.5, 'Type', 'bhp', ...
%    'Val', 0, 'name', 'P', 'radius', .1, 'Comp_i', [0 1]);    
W = addWell(W,G, rock, G.cells.num, 'Type', 'bhp', ...
    'Val', 20*barsa, 'name', 'P', 'radius', .1, 'Comp_i', [0 1]);
...
addpath 'C:\MRST\mrst-2019b\modules\book\examples\in2ph\utils'
% add path to obtain "getWellSol" function
...
for n=1:N
    ...
    fprintf(1,'\n');
    if n>4, continue, end % n>4 gives output only until 0.8 PV. To give output
                          % until 1.2 PV, then n>6 
    
    
    % Plot multiphase saturation solution
    figure(1);
    subplot(2,4,n);
    contourf(reshape(G.cells.centroids(:,1), G.cartDims),...
        reshape(G.cells.centroids(:,2), G.cartDims), ...
        reshape(x.s(:,1),G.cartDims), [0 cval 1]);
    ...
    contour(reshape(G.cells.centroids(:,1), G.cartDims),...
        reshape(G.cells.centroids(:,2), G.cartDims), ...
        reshape(tau/T,G.cartDims), a*n/N,'LineWidth',1);
    caxis([0 1]);
    ...
    % Plot multiphase pressure solution
    figure(2);
    subplot(2,4,n);
    contourf(reshape(G.cells.centroids(:,1), G.cartDims),...
        reshape(G.cells.centroids(:,2), G.cartDims), ...
        reshape(x.pressure(:,1)/barsa,G.cartDims));
    colorbar('southoutside'); 
    ...
    plot(tau(:,1)/tbf,x.s(:,1),'MarkerSize',4);
...
axis([0 max(t) -.05 1.0]);
title('Water Saturation in the Perforated Cell and Water Cut')
xlabel('Time [PV]'); ylabel('Saturation');
...
title('Oil Production Rate [m^3/d]')
xlabel('Time [PV]'); ylabel('Rate [m^3/d]');
...
title('Cumulative Oil Production [m^3]')
xlabel('time [PV]'); ylabel('Cum. Production [m^3]');